...
| `agent-rake --target <name>` | Write to a known agent file: `agents`, `claude`, `codex`, `copilot`, or `cursor`. |
...
The scanner reads bounded manifest data where needed, such as `package.json`.
Otherwise it works from file paths, names, and sizes, except for a small
route-declaration peek on likely entrypoint files below 128KB. Framework and
workspace signals come from manifests and conservative path conventions.
